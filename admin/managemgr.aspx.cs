...
public partial class admin_managemgr : System.Web.UI.Page
{
    Mgr objmgr = new Mgr(); 
    ...
    {
        this.gvPortfolio.DataSource = objmgr.GetMgr();  
        this.gvPortfolio.DataBind();
        if (this.gvPortfolio.Rows.Count == 0)
            message = "No Mgr found.";
    }
    ...
    protected void Button1_Click(object sender, EventArgs e)
    {
        ...
        try
        {
            ...
            int mid = int.Parse(btnDelete.CommandArgument);
            if (objmgr.DeleteMgrById(mid))   
            {
                message = "Mgr has been deleted successfully.";
            }
            else
            {
                message = "Unable to delete mgr.";
            }
        }
        catch (Exception ex)
        {
